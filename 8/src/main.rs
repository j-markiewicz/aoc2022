    ...
    for i in 0..trees.len() {
        for j in 0..trees[0].len() {
            if trees[i][j] > *trees[i][j + 1..].iter().max().unwrap_or(&i8::MIN) {
                n_visible += 1;
            } else if trees[i][j] > *trees[i][..j].iter().max().unwrap_or(&i8::MIN) {
                n_visible += 1;
            } else if trees[i][j] > trees[i + 1..].iter().map(|t| t[j]).max().unwrap_or(i8::MIN) {
                n_visible += 1;
            } else if trees[i][j] > trees[..i].iter().map(|t| t[j]).max().unwrap_or(i8::MIN) {
                n_visible += 1;
            }
        }
    }
